...
(function () {
  ...
  XH.storage = {
    ...
    normalize: function (state) {
      ...
      // 声音开关：旧存档补齐（缺省读旧 localStorage key，兼容更早的 xuehai_muted）
      if (typeof state.audioMuted !== 'boolean') {
        let legacy = false;
        try { legacy = localStorage.getItem('xuehai_muted') === '1'; } catch (e) { legacy = false; }
        state.audioMuted = legacy;
      }
